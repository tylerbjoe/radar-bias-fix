...
i=0
while i < len(bins):
    bins[i] = integrate.cumulative_trapezoid(bins[i])
    i+=1
...
# time_axis = np.arange(len(bins[0])) / 30
# # Create subplots
# fig, axes = plt.subplots(2, 1, sharex=True)  # 2 rows, 1 column

# # First subplot: Signal and Linear Model
# for i in range(len(bins)):
#     axes[0].plot(time_axis[5*30:31*30], bins[i][5*30:31*30], label=f"Integrated Bin {i}")

#     axes[0].set_ylabel('Displacement (unitless)', fontsize=16)

#     # axes[0].legend(loc='upper right', fontsize=16)#, bbox_to_anchor=(1, 0))#legend(loc='upper right')
#     axes[0].grid()
#     axes[0].set_title("Integrated Bins", fontsize=18)

# axes[1].plot(time_axis[5*30:31*30],sig[5*30:31*30]-interpolated_signal[5*30:31*30], label='Combined Bins')
# axes[1].set_ylabel('Displacement (unitless)', fontsize=16)
# axes[1].set_xlabel('Time (s)', fontsize=16)
# axes[1].tick_params(axis='x', labelsize=14)
# # axes[6].legend()
# axes[1].grid()
# axes[1].set_title('Combined Integrated Bins', fontsize=18)
