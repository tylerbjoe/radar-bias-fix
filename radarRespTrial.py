...
class Radar_Resp():
    ...
    def __init__(self):
        ...
        self.check_corr = 1 * self.fs   # rate to check if model still correlates to signal (s)
        self.corr_threshold = 0.7       # correlation threshold to refresh model
        self.window = np.zeros(10)               # empty window
    ...
    def set_model_point(self):
        self.model_point = self.slope * (self.streak / self.fs) + self.intercept
    ...
    def check_new_peak(self):
        last_peaks = find_peaks(np.negative(self.window), prominence=250)[0]#1650 prominence when breathold #250 for other??? distance=2*30,
        if len(last_peaks) != 0:
            x = self.sample_n - (self.win_size - last_peaks[-1])
            y = self.window[last_peaks[-1]]
            if x != self.lastnpeak[0]:
                self.slope = (y-self.lastnpeak[1])/x-(self.lastnpeak[0])
                self.lastnpeak=[x,y]
...
axes[0].plot(npeaks,sig[npeaks], label="LMS Model", color='orange')
axes[0].scatter(npeaks,sig[npeaks], label="LMS Model", color='orange')
